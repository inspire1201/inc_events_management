...
exports.getEvents = async (req, res) => {
  try {
    ...
    const [results] = await db.query('SELECT * FROM events WHERE status = ?', [status]);
    ...
    res.status(500).json({ error: 'डेटाबेस त्रुटि', details: error.message });
  }
};
...
exports.markEventAsViewed = async (req, res) => {
  try {
    ...
    await db.query(
      'INSERT INTO event_updates (user_id, event_id, updated_at) VALUES (?, ?, NOW()) ON DUPLICATE KEY UPDATE updated_at = NOW()',
      [user_id, event_id]
    );
    res.json({ message: 'इवेंट अपडेट सफलतापूर्वक दर्ज किया गया' });
  } catch (error) {
    console.error('markEventAsViewed error:', error);
    res.status(500).json({ error: 'डेटाबेस त्रुटि', details: error.message });
  }
};
...
exports.addEvent = async (req, res) => {
  try {
    const {
      event_name, date, description, photos, news_cuttings,
      social_links, courage_type, section
    } = req.body;

    await db.query(
      'INSERT INTO events (event_name, date, description, photos, news_cuttings, social_links, courage_type, status, section) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
      [event_name, date, description, photos, news_cuttings, social_links, courage_type, 'ongoing', section]
    );

    res.json({ message: 'इवेंट सफलतापूर्वक जोड़ा गया' });
  } catch (error) {
...
exports.updateEvent = async (req, res) => {
  try {
    const {
      event_id, present_people, courage_type,
      photos, news_cuttings, social_links
    } = req.body;

    await db.query(
      'UPDATE events SET present_people = ?, courage_type = ?, photos = ?, news_cuttings = ?, social_links = ?, status = ? WHERE id = ?',
      [present_people, courage_type, photos, news_cuttings, social_links, 'completed', event_id]
    );

    res.json({ message: 'इवेंट सफलतापूर्वक अपडेट किया गया' });
  } catch (error) {
...
exports.getEventReport = async (req, res) => {
  try {
    const [results] = await db.query('SELECT * FROM events WHERE status = ?', ['completed']);
    res.json(results);
  } catch (error) {
...
exports.getUserEventDetails = async (req, res) => {
  try {
    ...
    const [results] = await db.query(
      'SELECT e.*, eu.updated_at FROM events e JOIN event_updates eu ON e.id = eu.event_id WHERE eu.user_id = ?',
      [user_id]
    );
    res.json(results);
  } catch (error) {
    console.error('getUserEventDetails error:', error);
    res.status(500).json({ error: 'डेटाबेस त्रुटि', details: error.message });
  }
};
